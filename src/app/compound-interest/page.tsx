  ...
  return (
    <main style={{minHeight:"100vh",background:"#f8f9fb",fontFamily:"system-ui,sans-serif"}}>
      <nav style={{background:"#fff",borderBottom:"1px solid #eee",padding:"0 24px"}}>
        <div style={{maxWidth:1100,margin:"0 auto",height:56,display:"fl

cat > src/app/loan/page.tsx << 'ENDOFFILE'
"use client";
import { useState } from "react";
import Link from "next/link";
      ...
      <section style={{maxWidth:960,margin:"0 auto",padding:"48px 24px 80px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,alignItems:"start"}}>
          <div style={{background:"#fff",borderRadius:16,border:"1px solid #e5e7eb",padding:28}}>
            {[{label:"Loan amount ($)",val:amount,set:setAmount},{label:"Interest rate (%)",val:rate,set:setRate},{label:"Loan term (years)",val:years,set:setYears}].map((f)=>(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {[["Total payment",fmt(total)],["Total interest",fmt(interest)],["Loan amount",fmt(a)]].map(([l,v])=>(
